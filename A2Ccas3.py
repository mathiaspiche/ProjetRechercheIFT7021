...
def plot_all_average_rewards(results):
    plt.figure(figsize=(10, 6))

    for result in results:
        plt.plot(
            result["block_episodes"],
            result["block_avg_rewards"],
            label=result["grid_name"]
        )

    plt.xlabel("Épisode")
    plt.ylabel("Récompense cumulative moyenne")
    plt.title("Récompense cumulative moyenne par bloc de 100 épisodes")
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
plot_all_average_rewards(results)
